...
`````{verbatim, lang = "markdown", code = verbatim_content_knitted}
`````
]
]
...
.ba[
.title-file[.Rmd before knitting]
`````{verbatim, lang = "markdown", code = gsub("(`{3}\\{)", "*\\1", embed_content)}
`````
]
...
.ba.mt2[
.title-file[.md after knitting]
```{verbatim, code = embed_content_knitted}
```
]
